...
const Home = () => {
	...
	return (
		<section className='relative w-full min-h-screen overflow-hidden'>
			<Particles className='absolute inset-0 z-0' color='var(--background)' />
			...
			<div className='relative z-10 flex flex-col justify-center items-center w-full min-h-screen gap-6 lg:gap-10 px-4 py-12'>
				<h1 className='font-bold flex text-4xl lg:text-7xl bg-clip-text text-center animate-in fade-in duration-700'>
					<span className=' rounded-sm line-through bg-accent-foreground italic block text-muted-foreground'>Task</span>
				...
				<p className='text-muted-foreground text-center max-w-2xl text-base lg:text-xl mt-4 leading-relaxed'>
					Organize your life with smart task management. Capture, prioritize,
					and complete your to-dos with ease.
				</p>

				<div className='flex flex-col sm:flex-row gap-4 mt-6'>
					<Button asChild size="lg" className='text-lg font-semibold px-8 py-4 h-auto backdrop-blur-sm bg-primary/90 hover:bg-primary/80'>
						<Link href='/signup'>
							Get Started Free
						</Link>
					</Button>
					<Button asChild variant="outline" size="lg" className='text-lg font-semibold px-8 py-4 h-auto backdrop-blur-sm bg-background/20 border-primary/30 hover:bg-accent/50'>
						<Link href='/explore'>
							See How It Works
						</Link>
					</Button>
				...
				<div className='grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 mt-16 max-w-5xl w-full'>
					<Card className='text-center hover:shadow-lg hover:scale-105 transition-all duration-300 cursor-pointer border-primary/20 hover:border-primary/40 backdrop-blur-sm bg-card/50'>
						<CardHeader>
							<CardTitle className='flex justify-center text-5xl mb-4'>✓</CardTitle>
